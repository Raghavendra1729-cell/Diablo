...
# Google Form Bulk Submitter

Bulk-submit prepared rows to the Buddy Program Google Form.

Form:
https://docs.google.com/forms/d/e/1FAIpQLSciKuH9cpj7PkUn0pMDb18T9_oVDzrzEXR-6M8G_Kn3B7Dh3w/viewform

## Files

- `responses_template.csv`: Fill this file. One row equals one form response.
- `submit_google_form.py`: Validates and submits CSV rows.
- `form_config.json`: Required config with Google Form entry IDs, valid options, and buddy/student mappings.
- `README.md`: This guide.

## Basic Workflow

1. Open `responses_template.csv`.
2. Add one response per row.
3. Keep the header row and column names unchanged.
4. Run a dry-run validation.
5. Submit one row first and verify the Google Sheet.
6. Submit all rows only after verification.

## Commands

Dry-run validation only. This does not submit anything.

```bash
./submit_google_form.py responses_template.csv
```

Show the exact form payload that would be submitted.

```bash
./submit_google_form.py responses_template.csv --show-payload
```

Submit only the first row. Use this for testing.

```bash
./submit_google_form.py responses_template.csv --submit --limit 1
```

Submit all rows.

```bash
./submit_google_form.py responses_template.csv --submit
```

Use a different CSV file.

```bash
./submit_google_form.py my_responses.csv --submit
```

## CSV Rules

- `date`, `assign_task_follow_up_date`, `incomplete_task_follow_up_date`, and `psp_follow_up_date` accept `YYYY-MM-DD`, `DD/MM/YYYY`, or `DD-MM-YYYY`.
- Multi-select columns use semicolon-separated values, for example: `Data Analytics;Operating Systems & Concurrency`.
- Leave conditional fields blank when they do not apply.
- Use exact option labels. The script validates labels before submitting.
- `buddy_name` must match a buddy in the form.
- `student_name` must be in that buddy's student list.
...
- If `wellbeing_concern` is `Yes`, fill `wellbeing_concern_details` and `psychological_counsellor_escalation`.
- If `academic_status` is `Slightly Behind` or `Majorly Behind`, fill `academic_struggle_subjects` and `academic_struggle_details`.
- If `assigning_tasks` is `Yes`, fill task subject/details/follow-up date.
- If `last_week_task_completed` is `Partially` or `No`, fill incomplete task fields.
- If `psp_goal_met` is `Partially` or `No`, fill PSP blocker fields.
- If `parallel_degree_issues` is `Yes`, fill parallel degree fields.

## Allowed Values

Common option values are listed below. For full buddy/student mappings, see `form_config.json`.

- `session_type`: `P0 Weekly 1:1`, `P1 Monthly 1:1`, `Ad-hoc Check-in`, `Missed Session (student absent)`
- `wellbeing_rating`: `1`, `2`, `3`, `4`, `5`
- `wellbeing_concern`: `Yes`, `No`
- `psychological_counsellor_escalation`: `Yes`, `No`, `Maybe`
- `academic_status`: `On Track`, `Slightly Behind`, `Majorly Behind`
- Subject fields: `Introduction to Database Management Systems`, `Operating Systems & Concurrency`, `Data Analytics`
- `assigning_tasks`: `Yes`, `No`
- `last_week_task_completed`: `Yes`, `Partially`, `No`, `NA`
- `psp_goal_met`: `Yes`, `Partially`, `No`, `NA`
- `psp_follow_up_required`: `Yes`, `No`
- `psp_instructor_escalation`: `Yes (critical)`, `Yes (not urgent)`, `No`
- `psp_instructor_to_notify`: `Saurabh`, `Chitwan`, `Ayush`, `Akanksha`
- `parallel_degree_issues`: `Yes`, `No`, `NA`
- `parallel_degree`: `BITs`, `IIT - M`
- `parallel_degree_raise_to_instructor`: `Yes`, `No`

## AI Prompt For Preparing CSV Rows

Use this with your session notes:

```text
Given my rough session notes below, produce one CSV row using exactly the columns from responses_template.csv.
Use exact option labels from README.md.
Use semicolon-separated values for multi-select fields.
Leave non-applicable conditional fields blank.
...
The script defaults to dry-run. Nothing is submitted unless you pass `--submit`.
